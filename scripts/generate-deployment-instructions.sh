...
EOF
  ...
  if [ -n "${DB_NAME:-}" ] && [ -f ${INFRA_TMP_DIR}/db-env-options.json ]; then
    cat <<EOF
...
EOF
    ...
    if [ -f ${INFRA_TMP_DIR}/custom-domain.txt ]; then
        local custom_domain=$(cat ${INFRA_TMP_DIR}/custom-domain.txt)
        cat <<EOF
...
${GREEN}────────────────────────────────────────────────────────────────────────${NC}
  ...
  1. Verify certificate status:
     aws acm describe-certificate \\
       --certificate-arn \$(cat ${INFRA_TMP_DIR}/acm-cert-arn.txt 2>/dev/null || echo "YOUR_CERT_ARN") \\
       --profile $AWS_PROFILE \\
       --region $AWS_REGION
